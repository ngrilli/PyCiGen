...
c with coordinates in the midsurface reference frame
     ...
     & delta0,deltaF,delu_loc_gp,mcrd,nrhs,SVARS,damage(i))
...
c
     ...
     & delta0,deltaF,delu,mcrd,nrhs,SVARS,damage)
...
c
      ...
      elseif (damage .GE. 1.0) then ! completely damaged
        ...
        if (popn .LT. 0.0) then ! crack open
          Tn = 0.0
          Dnn = 0.0
        else ! contact
          Tn = stiffK*popn
        ...
        Tt = 0.0 ! shear traction always 0 when completely damaged
        Dtt = 0.0

      else ! partially damaged

        Tt = damage*stiffG*popt
        Dtt = damage*stiffG

        ! deltaD is the critical opening at which the
        ! traction behaviour changes from hardening to softening
        deltaD = delta0 + damage*(deltaF-delta0)

        ! maxS is the max stress reachable at this
        ! level of damage
        maxS = sigma0*(1.0-damage)

        if (popn .GE. deltaD) then ! softening
          Tn = maxS*(deltaF-popn)/(deltaF-deltaD)
          Dnn = -maxS/(deltaF-deltaD)
        else ! hardening
          Tn = maxS*popn/deltaD
          Dnn = maxS/deltaD
        end if
      ...
      ! no coupling between normal and shear force/displacement
      Dnt = 0.0
...
c
         T_d(2,1)=Dtt*delu(1)*delu(2)/(popt**2.0)
     &    -Tt*delu(1)*delu(2)/(popt**3.0)
...
! Damage accumulation
      if (damage .LE. 0.0) then ! undamaged
        if (abs(popn) .GT. delta0) then
          damage = damage + (abs(popn)-delta0)/(deltaF-delta0)
        end if
      else ! partially damaged
        ...
        if (abs(popn) .GT. deltaD) then
          damage = damage + (abs(popn)-deltaD)/(deltaF-delta0)
        end if

      end if
      
      if (damage .GT. 1.0) then ! limit damage to 1.0
        damage = 1.0
      end if
